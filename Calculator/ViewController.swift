...
//        if self.math!.text == "0" {
//            self.math!.text = value
//        } else {
//            
//        }
        
        if (self.rst?.text != nil && (self.rst?.text!.count)! > 0) {
            var rstStr = self.rst!.text
            rstStr = rstStr!.replacingOccurrences(of: "=", with: "")
            self.math!.text = rstStr!.appending(value)
            self.rst?.text = nil
        } else {
            self.math!.text = self.math!.text?.appending(value) ?? value
        }
    }
    ...
    func didChangeMathOperator(value: String) {
        var text = self.math!.text!
        let start = text.index(text.endIndex, offsetBy: -1)
